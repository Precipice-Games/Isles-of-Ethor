...
private void ApplySingleGust(SelectableTile gustTile)
{
    ...
    switch (gustTile.gustDirection)
    {
        case SelectableTile.GustDirection.Up:
            dirZ = 1;
            break;
        case SelectableTile.GustDirection.Down:
            dirZ = -1;
            break;
        case SelectableTile.GustDirection.Left:
            dirX = -1;
            break;
        case SelectableTile.GustDirection.Right:
            dirX = 1;
            break;
    }

    int x = gustTile.gridX + dirX;
    int z = gustTile.gridZ + dirZ;

    while (IsInsideGrid(x, z))
    {
        if (grid[x, z] != null)
        {
            PushTile(grid[x, z], dirX, dirZ);
        }
